...
import org.jhotdraw.samples.svg.io.commands.ToPath;
...
// XXX - Handle "auto" width and height
    ...
    private BezierPath[] toPath(IXMLElement elem, String str) throws IOException {
        ...
        StreamPosTokenizer streamTokenizer;
        if (toPathTokenizer == null) {
            streamTokenizer = new StreamPosTokenizer(new StringReader(str));
            streamTokenizer.resetSyntax();
            streamTokenizer.parseNumbers();
            streamTokenizer.parseExponents();
            streamTokenizer.parsePlusAsNumber();
            streamTokenizer.whitespaceChars(0, ' ');
            streamTokenizer.whitespaceChars(',', ',');
            toPathTokenizer = streamTokenizer;
        } else {
            streamTokenizer = toPathTokenizer;
            streamTokenizer.setReader(new StringReader(str));
        }
        ...
        char command = 'M';
        Commands:
        while (streamTokenizer.nextToken() != StreamPosTokenizer.TT_EOF) {
            if (streamTokenizer.ttype > 0) {
                command = (char) streamTokenizer.ttype;
            } else {
                command = nextCommand;
            ...
            switch (command) {
                case 'M':
                    ...
                    path = new BezierPath();
                    nextCommand = ToPath.absoluteMoveto(streamTokenizer, str, p, path);
                    break;
                case 'm':
                    ...
                    path = new BezierPath();
                    nextCommand = ToPath.relativeMoveto(streamTokenizer, str, p, path);
                    break;
                case 'Z':
                case 'z':
                ...
                case 'L':
                    nextCommand = ToPath.absoluteLineto(streamTokenizer, str, p, path);
                    break;
                    
                case 'l':
                    nextCommand = ToPath.relativeLineto(streamTokenizer, str, p, path);
                    break;
                    
                case 'H':
                    nextCommand = ToPath.absoluteHorizontalLineto(streamTokenizer, str, p, path);
                    break;
                    
                case 'h':
                    nextCommand = ToPath.relativeHorizontalLineto(streamTokenizer, str, p, path);
                    break;
                    
                case 'V':
                    nextCommand = ToPath.absoluteVerticalLineto(streamTokenizer, str, p, path);
                    break;
                    
                case 'v':
                    nextCommand = ToPath.relativeVerticalLineto(streamTokenizer, str, p, path);

                    break;
                case 'C':
                    nextCommand = ToPath.absoluteCurveto(streamTokenizer, str, c1, c2, p, path);
                    break;

                case 'c':
                    nextCommand = ToPath.relativeCurveto(streamTokenizer, str, c1, p, c2, path);
                    break;

                case 'S':
                    nextCommand = ToPath.absoluteShorthandCurveto(path, c1, streamTokenizer, str, c2, p);
                    break;

                case 's':
                    nextCommand = ToPath.relativeShorthandCurveto(path, c1, streamTokenizer, str, c2, p);
                    break;

                case 'Q':
                    nextCommand = ToPath.absoluteQuadto(streamTokenizer, str, c1, p, path);
                    break;
                case 'q':
                    nextCommand = ToPath.relativeQuadto(streamTokenizer, str, c1, p, path);
                    break;
                case 'T':
                    nextCommand = ToPath.absoluteShorthandQuadto(path, c1, streamTokenizer, str, p);
                    break;
                case 't':
                    nextCommand = ToPath.relativeShorthandQuadto(path, c1, streamTokenizer, str, p);
                    break;
                case 'A': {
                    nextCommand = ToPath.absoluteEllipticalArc(streamTokenizer, str, p, path);
                    break;
                }
                case 'a': {
                    nextCommand = ToPath.relativeEllipticalArc(streamTokenizer, str, p, path);
                    break;
                }
                default:
                    if (DEBUG) {
