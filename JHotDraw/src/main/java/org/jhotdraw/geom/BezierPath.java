...
public class BezierPath extends ArrayList<BezierPath.Node>
    ...
    public Rectangle2D.Double getBounds2D() {
        if (bounds == null) {
            int size = this.size();
            if (size == 0) {
                LowerBoundX = LowerBoundY = UpperBoundX = UpperBoundY = 0.0f;
            } else {
                double x, y;

                // handle first node
                Node node = get(0);
                LowerBoundY = UpperBoundY = node.y[0];
                LowerBoundX = UpperBoundX = node.x[0];
            ...
            bounds = new Rectangle2D.Double(LowerBoundX, LowerBoundY, UpperBoundX - LowerBoundX, UpperBoundY - LowerBoundY);
        }
